...
@Component({
    selector: 'jhi-purchase-item-mysuffix',
    templateUrl: './purchase-item-mysuffix.component.html'
})
export class PurchaseItemMysuffixComponent implements OnInit, OnDestroy {

currentAccount: any;
    purchaseItems: PurchaseItemMysuffix[];
    error: any;
    success: any;
    ...
    loadAll() {
        this.purchaseItemService.query({
            page: this.page - 1,
    ...
    transition() {
        this.router.navigate(['/purchase-item-mysuffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/purchase-item-mysuffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: PurchaseItemMysuffix) {
        return item.id;
    }
