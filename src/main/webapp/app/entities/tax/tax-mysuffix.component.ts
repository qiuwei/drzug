...
@Component({
    selector: 'jhi-tax-mysuffix',
    templateUrl: './tax-mysuffix.component.html'
})
export class TaxMysuffixComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/tax-mysuffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/tax-mysuffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: TaxMysuffix) {
        return item.id;
    }
