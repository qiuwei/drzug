...
@Component({
    selector: 'jhi-order-item-mysuffix',
    templateUrl: './order-item-mysuffix.component.html'
})
export class OrderItemMysuffixComponent implements OnInit, OnDestroy {

currentAccount: any;
    orderItems: OrderItemMysuffix[];
    error: any;
    success: any;
    ...
    transition() {
        this.router.navigate(['/order-item-mysuffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/order-item-mysuffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: OrderItemMysuffix) {
        return item.id;
    }
