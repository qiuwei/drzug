...
@Component({
    selector: 'jhi-customer-mysuffix',
    templateUrl: './customer-mysuffix.component.html'
})
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/customer-mysuffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/customer-mysuffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
