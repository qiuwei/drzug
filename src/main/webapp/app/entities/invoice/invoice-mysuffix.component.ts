...
@Component({
    selector: 'jhi-invoice-mysuffix',
    templateUrl: './invoice-mysuffix.component.html'
})
export class InvoiceMysuffixComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/invoice-mysuffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/invoice-mysuffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: InvoiceMysuffix) {
        return item.id;
    }
