...
@Component({
    selector: 'jhi-purchase-mysuffix',
    templateUrl: './purchase-mysuffix.component.html'
})
export class PurchaseMysuffixComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/purchase-mysuffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/purchase-mysuffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: PurchaseMysuffix) {
        return item.id;
    }
