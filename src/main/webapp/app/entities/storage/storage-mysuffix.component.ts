...
@Component({
    selector: 'jhi-storage-mysuffix',
    templateUrl: './storage-mysuffix.component.html'
})
export class StorageMysuffixComponent implements OnInit, OnDestroy {

currentAccount: any;
    storages: StorageMysuffix[];
    error: any;
    success: any;
    ...
    transition() {
        this.router.navigate(['/storage-mysuffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/storage-mysuffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: StorageMysuffix) {
        return item.id;
    }
