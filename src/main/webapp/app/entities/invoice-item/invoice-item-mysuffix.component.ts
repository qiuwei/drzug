...
@Component({
    selector: 'jhi-invoice-item-mysuffix',
    templateUrl: './invoice-item-mysuffix.component.html'
})
export class InvoiceItemMysuffixComponent implements OnInit, OnDestroy {

currentAccount: any;
    invoiceItems: InvoiceItemMysuffix[];
    error: any;
    success: any;
    ...
    transition() {
        this.router.navigate(['/invoice-item-mysuffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/invoice-item-mysuffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: InvoiceItemMysuffix) {
        return item.id;
    }
