...
@Component({
    selector: 'jhi-payment-mysuffix',
    templateUrl: './payment-mysuffix.component.html'
})
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/payment-mysuffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/payment-mysuffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
