...
@Component({
    selector: 'jhi-order-mysuffix',
    templateUrl: './order-mysuffix.component.html'
})
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/order-mysuffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/order-mysuffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: OrderMysuffix) {
        return item.id;
    }
