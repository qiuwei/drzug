...
@Component({
    selector: 'jhi-product-mysuffix',
    templateUrl: './product-mysuffix.component.html'
})
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/product-mysuffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/product-mysuffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: ProductMysuffix) {
        return item.id;
    }
